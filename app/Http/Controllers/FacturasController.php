...
class ClientOrderController extends Controller{
    ...
    public function createHeader(Request $request){
        ...
        $query = "INSERT INTO F_PCL(TIPFAC, CODFAC, REFFAC, FECFAC, AGEFAC, CLIFAC, ALMFAC, CNOFAC, NET1FAC, BAS1FAC, PIVA1FAC, IIVA1FAC, TOTFAC, FPOFAC, HORFAC) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)";
        ...
        $serie = $this->getSerieToRequisition($requisition->from->id);
        $code = $this->nextOrder($serie);
        $total = array_reduce($request->products, function($total, $product){
            return $total + $product["ordered"]["toDelivered"] * $product["ordered"]["price"];
        }, 0);
        $result = $exec->execute([$serie, $code, "P-".$request->id, $date_format, $request->created_by["id"], $request->from["_client"], 'GEN', $request->from['name'], $total, $total, 0, 0, $total,"CRE", $hour]);
        if($result){
            /* Crear detalles del pedido (productos) */
            $res = $this->createBody($serie, $code, $request->products);
            if($res){
                return response()->json(["msg" => "creado", "status" => 200, "serie" => $serie,"ticket" => $code]);
            }else{
    ...
    public function createBody($cash, $code, $body){
        $query = "INSERT INTO F_LPC(TIPLFA, CODLFA, POSLFA, ARTLFA, DESLFA, CANLFA, PRELFA, COSLPC, TOTLFA) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $exec = $this->con->prepare($query);
        $success = true;
        $success_stock = true;
        $counter = 1;
        foreach($body as $key => $row){
            if($row["ordered"]["toDelivered"] && $row["ordered"]>0){
                $result = $exec->execute([$cash, $code, $counter, $row["code"], $row["description"], $row["ordered"]["toDelivered"], $row["ordered"]["price"], $row["cost"], $row["ordered"]["total"]]);
    ...
    public function reserveStock($code, $amount, $reserve = true){
        ...
        $query_update = "UPDATE F_STO SET DISSTO = ?, ACTSTO = ? WHERE ALMSTO = 'GEN' AND ARTSTO = ?";
